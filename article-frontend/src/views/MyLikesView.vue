<template>
  <div class="my-likes-view">
    <el-page-header @back="$router.push('/')" title="返回首页">
      <template #content><span class="page-title">我的点赞</span></template>
    </el-page-header>

    <div class="content-area" v-loading="loading">
      <!-- 点赞列表 -->
      <div v-if="list.length > 0" class="like-list">
        <el-card v-for="item in list" :key="item.id" class="like-card" shadow="hover" @click="$router.push(`/article/${item.id}`)">
          <div class="card-body">
            <div class="card-info">
              ...
              <div class="card-meta">
                ...
                <span class="meta-text"><el-icon><StarFilled /></el-icon> {{ item.likeCount || 0 }} 人点赞</span>
                <span class="meta-time">点赞于 {{ formatTime(item.likedAt || item.createdAt) }}</span>
              </div>
            </div>
            <el-button type="danger" text size="small" class="unlike-btn" @click.stop="handleUnlike(item)">
              取消点赞
            </el-button>
          </div>
        </el-card>
      ...
      <el-empty v-else description="还没有点赞任何文章，去发现好文吧！">
